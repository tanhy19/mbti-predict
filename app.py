...
st.markdown("Enter a paragraph or meaningful sentence to predict your MBTI type.")
    ...
    if not is_valid_input(user_input):
        ...
        with col2:
            if mode == "Ensemble":
                ...
                st.metric("🧩 Model Used", "Ensemble")
                st.metric("📊 Avg Accuracy", f"{avg_acc*100:.1f}%")
            else:
        ...
        with st.expander("🧩 MBTI Breakdown"):
            for i, dim in enumerate(["I/E", "N/S", "F/T", "P/J"]):
                letter = preds[i]
                st.metric(dim, letter, mbti_descriptions[letter])
                st.progress(int(confidences[dim] * 100))
...
st.markdown("---")
st.markdown("<small>Powered by an ensemble of Naive Bayes, SVM, and Random Forest models.</small>", unsafe_allow_html=True)
